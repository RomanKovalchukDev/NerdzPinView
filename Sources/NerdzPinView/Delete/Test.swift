...
//  NerdzPinView
//
//  Created by Roman Kovalchuk on 17.02.2025.
//

import UIKit

@MainActor
public final class OneTimeCodeTextField: UIControl {
...
private extension OneTimeCodeTextField {
    ...
    func updateAccessibilityProperties() {
        ...
        accessibilityHint = isFirstResponder
            ? nil
            : "Hint"// STPLocalizedString("Double tap to edit", "Accessibility hint for a text field")
    }
...
//            focusRing.isHidden = !isActive
        ...
        private func updateColors() {
            borderLayer.backgroundColor = UIColor.red.cgColor// isEnabled ? theme.colors.componentBackground.cgColor : theme.colors.disabledBackground.cgColor
...
public struct ElementsAppearance {

    /// The default appearance used for Elements
    @MainActor public static let `default` = ElementsAppearance()

    public var fonts = Font()
    public var colors = Color()

    public var borderWidth = 1
    public var cornerRadius = 8
    public var shadow: Shadow? = Shadow()

    public struct Font {
        public init() {}

        public var subheadline = UIFont.systemFont(ofSize: 14)
        public var subheadlineBold = UIFontMetrics(forTextStyle: .body).scaledFont(for: .systemFont(ofSize: 14, weight: .bold))
        public var sectionHeader = UIFont.systemFont(ofSize: 14)
        public var caption = UIFont.systemFont(ofSize: 12, weight: .regular)
        public var footnote = UIFont.systemFont(ofSize: 14)
        public var error = UIFont.systemFont(ofSize: 14)
        public var smallFootnote = UIFont.systemFont(ofSize: 14)
        public var footnoteEmphasis = UIFont.systemFont(ofSize: 14)
    }
    ...
    public struct Color {
        ...
        public var componentBackground = UIColor.white
        public var disabledBackground = UIColor.lightGray
        public var border = UIColor.black
        public var divider = UIColor.gray
...
@MainActor
final class ActiveIndexStorage {
    ...
    var value: String {
        ...
        set {
            let newValueArray = Array(newValue)
            
            for index in 0..<characters.count {
                if index < newValueArray.count {
                    characters[index] = newValueArray[index]
                }
                else {
    ...
    // Value that indicates if value filled
    var isFull: Bool {
    ...
    var activeIndexTextPosition: TextPosition {
        TextPosition(activeIndex)
    }
    
    var activeIndexTextRange: TextRange {
        TextRange(start: activeIndexTextPosition, end: activeIndexTextPosition)
    }
    
    var minTextPosition: TextPosition {
        TextPosition(0)
    }
    
    var maxTextPosition: TextPosition {
        TextPosition(capacity)
    }
    
    var maxPossibleRange: TextRange {
        TextRange(start: minTextPosition, end: maxTextPosition)
    }
    
    // The first position of filled value
    var valueStartPosition: TextPosition {
        let offset = characters.firstIndex(where: { $0 != nil }) ?? .zero
        return TextPosition(offset)
    }
    
    // The latest filled position
    var valueEndPosition: TextPosition {
        let offset = characters.lastIndex(where: { $0 != nil }) ?? max((value.count - 1), 0)
        return TextPosition(offset)
    }
    ...
    init(capacity: Int) {
        ...
        self.capacity = max(capacity, 0)
        self.characters = (0..<capacity).map({ _ in nil })
    }
    
    func insert(_ text: String, at range: TextRange) -> TextRange {
        let lowerBound = min(range._start.index, range._end.index)
        let upperBound = max(range._start.index, range._end.index)
        
        var filteredText = text
            .filter({ $0.unicodeScalars.allSatisfy(allowedCharacters.contains(_:)) })
        
        if (filteredText.count + lowerBound) > capacity {
            filteredText.prefix(capacity - lowerBound)
        }
        
        let filteredTextArray: [Character] = Array(filteredText)
        
        var filterTextIterator: Int = .zero
        for index in lowerBound...upperBound {
            if filterTextIterator < filteredText.count {
                characters[index] = filteredTextArray[filterTextIterator]
            }
            
            filterTextIterator += 1
        }
        
        let nextInsertionIndex = min((range._start.index + filteredText.count), capacity - 1)
        let nextInsertionPoint = TextPosition(nextInsertionIndex)
        return TextRange(start: nextInsertionPoint, end: nextInsertionPoint)
    }
    ...
    func delete(range: TextRange) -> TextRange {
        ...
        var currentValueInRange: [Character?] = []
        
        for index in lowerBound...upperBound {
            currentValueInRange.append(characters[index])
            characters[index] = nil
        }
        
        let prevIndex = max(range._start.index - 1, 0)
        return TextRange(start: TextPosition(prevIndex), end: TextPosition(prevIndex))
    }
    ...
    func makeRange(from start: TextPosition, to end: TextPosition) -> TextRange? {
        guard maxPossibleRange.contains(start.index), maxPossibleRange.contains(end.index) else {
            return nil
        }
...
@MainActor
public class PinCodeInputView3<T: DigitViewType>: UIView, UIKeyInput, @preconcurrency UIEditMenuInteractionDelegate, UITextInput {
    ...
    private var textStorage: ActiveIndexStorage = ActiveIndexStorage(capacity: .zero)
    ...
    open func insertText(_ text: String) {
        ...
        debugPrint(range.description, #function, selectedTextRange)
    ...
    open override func becomeFirstResponder() -> Bool {
        ...
        if result {
            selectedTextRange = textStorage.activeIndexTextRange
    ...
    private func configureView() {
        self.textStorage = ActiveIndexStorage(capacity: config.pinLength)
        self.itemViews = (0..<config.pinLength).map { index in
            let view = T()
            ...
            view.onViewTapped = { [weak self] in
                self?.textStorage.activeIndex = index
                self?.becomeFirstResponder()
                self?.update()
            }
    ...
    private func updateItemViews() {
        let digits: [Character?] = textStorage.characters
        let selectedRange = selectedTextRange as? TextRange
    ...
    public var selectedTextRange: UITextRange? = nil {
        ...
        didSet {
            inputDelegate?.selectionDidChange(self)
//            debugPrint("Selected was set", selectedTextRange?.description)
            update()
        }
    }
    ...
    public var beginningOfDocument: UITextPosition {
        return textStorage.minTextPosition
    }
    
    public var endOfDocument: UITextPosition {
        return textStorage.maxTextPosition
    }
    ...
    public func position(from position: UITextPosition, offset: Int) -> UITextPosition? {
        ...
        guard textStorage.maxPossibleRange.contains(newIndex) else {
            // Out of bounds
            return nil
        }
    ...
    public func closestPosition(to point: CGPoint) -> UITextPosition? {
        return closestPosition(to: point, within: textStorage.maxPossibleRange)
    }
