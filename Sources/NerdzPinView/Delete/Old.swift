...
@MainActor
public class PinCodeInputViewOld<T: UIView & PinCodeItemViewType & PinCodeItemLayoutConfigurable & PinCodeItemAppearanceConfigurable>:
    UIView,
    UIKeyInput,
    ...
    private var activeItemIndex: Int? {
        didSet {
            let lastIndex = max((activeItemIndex ?? .zero), config.pinLength)
            selectedTextRange = TextRange(start: TextPosition(lastIndex), end: TextPosition(lastIndex))
        }
    }
...
//        }
    ...
    public func text(in range: UITextRange) -> String? {
        ...
        let stringRange = range.stringRange(for: text)
    ...
    public func position(from position: UITextPosition, offset: Int) -> UITextPosition? {
        guard let from = position as? TextPosition else {
            return nil
        }
        ...
        let newOffset = max(min(from.index + offset, text.count), 0)
        return TextPosition(newOffset)
    }
    ...
    public func compare(_ position: UITextPosition, to other: UITextPosition) -> ComparisonResult {
        guard let from = position as? TextPosition, let to = other as? TextPosition else {
            return .orderedSame
        }
    ...
    public func characterRange(byExtending position: UITextPosition, in direction: UITextLayoutDirection) -> UITextRange? {
        guard let myPosition = position as? TextPosition else {
            return nil
        }
        
        return TextRange(start: myPosition, end: TextPosition(text.count))
    }
